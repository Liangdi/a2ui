...
```
┌──────────────────────────────────────────────────────────────┐
│  apps:       a2ui-gallery (TUI)        a2ui-slint-gallery (桌面)│
├──────────────────────────────────────────────────────────────┤
│  umbrella:   a2ui  (re-export core + tui [+ slint])           │
├──────────────────────────────────────────────────────────────┤
│  backends:   a2ui-tui (ratatui)        a2ui-slint (Slint, 可选)│
├──────────────────────────────────────────────────────────────┤
│  a2ui-base (框架无关:Protocol / Model / Catalog / Processor)  │
└──────────────────────────────────────────────────────────────┘
```

依赖自下而上:`a2ui-base` 同时支撑两个后端 —— `a2ui-tui`(ratatui,默认)与 `a2ui-slint`(Slint 桌面,可选)。`a2ui-tui` ← `a2ui-gallery`;`a2ui-slint` ← `a2ui-slint-gallery`;`a2ui`(umbrella)依赖 core + tui(slint 在 `slint` feature 后)。`a2ui-base` 完全不依赖 ratatui/slint,可独立用于其他 backend。
...
│   └── src/main.rs
└── a2ui/              # a2ui：umbrella，re-export core+tui [+slint]
    ├── src/lib.rs
    └── examples/                  # 17 个示例
```
