...
fn main() {
    ...
    slint_build::compile(dst.to_str().expect("utf8 path")).expect("Slint UI compilation failed");
}
...
fn generate() -> String {
    ...
    out.push_str(
        "// Bounded-depth unrolling of the A2UI component tree (Slint cannot recurse).\n\n",
    );
...
fn node_body(k: usize, child_comp: Option<&str>) -> String {
    ...
    let render_children_v = match child_comp {
        Some(c) => {
            format!("for c in me.children : {c} {{ all: all; idx: c; vertical-stretch: 0; }}")
        }
        None => "Text { text: \"…\"; color: #888; }".to_string(),
    };
    let render_children_h = match child_comp {
        Some(c) => {
            format!("for c in me.children : {c} {{ all: all; idx: c; horizontal-stretch: 0; }}")
        }
        None => "Text { text: \"…\"; color: #888; }".to_string(),
    };
